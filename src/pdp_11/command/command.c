 ...
 * команды, вырывая из него суть: информацию о том, "откуда" (Source) придут
 * данные и "куда" (Destination) они отправятся после исполнения.
 *
 * @param[in,out] pdp  Указатель на святая святых — структуру эмулятора @ref
 * pdp_11_t. Может измениться, если аргументы потребуют автоинкремента
 * регистров.
 ...
 *
 * @warning
 * Функция вызывает @ref __get_args, что может привести к побочным эффектам:
 * изменению указателя стека или программного счетчика (PC) при использовании
 * косвенной адресации!
 *
 * @code
