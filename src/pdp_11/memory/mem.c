...
static inline address_word_t
__corect_addr_for_write_word(address_word_t addr)
{
    if ((addr & 1)) {
        addr ^= 1;
        return addr;
...
//----------------------------------------------------------------------
void
byte_write(mem_t *memory, address_byte_t addr, byte_t data)
{
...
byte_t
byte_read(mem_t *memory, address_byte_t addr)
{
    assert(__is_valid_addr(memory, addr, 1));
    if (!(memory->default_memory)) {
    ...
    return (byte_t) memory->mem_word->read_byte(memory->mem_word, addr);
}
//#######################################################
void
word_write(mem_t *memory, address_word_t addr, word_t data)
{
    addr = __corect_addr_for_write_word(addr);
    assert(__is_valid_addr(memory, addr, 2));

    if (!(memory->default_memory)) {
        memory->mem_byte->write_word(memory->mem_byte, addr, data);
        return;
    }
...
word_t
word_read(mem_t *memory, address_word_t addr)
{
    assert(__is_valid_addr(memory, addr, 2));
    if (!(memory->default_memory)) {
        return (word_t) memory->mem_byte->read_word(memory->mem_byte, addr);
    }
    if (addr & 1) {
        addr &= -1;
    }
