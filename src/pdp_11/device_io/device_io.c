...
dev_io_new()
{
    return (dev_io_t *) malloc(
        sizeof(dev_io_t)); // Выделение паамяти для обЪекта dev_io_t
}

void
dev_io_create(dev_io_t *device_io)
{
    device_io->default_device = g_default_device;
}

void
dev_io_destroy(dev_io_t *device_io)
{
...
void
dev_io_load_data(struct pdp_11_t *pdp, dev_io_t *device_io, byte_t *filename)
{
    if (filename) {
