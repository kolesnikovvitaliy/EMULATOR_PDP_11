...
pdp_new()
{
    return (pdp_11_t *) malloc(
        sizeof(pdp_11_t)); // Выделение памяти под обЬетк PDP_11;
}
//------------------------------------------------------------------;
void
pdp_create(pdp_11_t *pdp)
{
    pdp->memory = (struct mem_t *)
        mem_new(); // Выделение памяти под обЬетк  памяти типа byte и word;
    pdp->device_io = (struct dev_io_t *)
        dev_io_new(); // Выделение памяти под обЬетк ввода вывода;
    pdp->regist = (struct reg_t *)
        register_new(); // Выделение памяти под обЬетк Регистры;
    ...
    reg_create((struct pdp_11_t *) pdp,
               pdp->regist); // Инициализация обЬекта Регистры
...
pdp_destroy(pdp_11_t *pdp)
{
    ...
    dev_io_destroy((struct dev_io_t *)
                       pdp->device_io); // Уничтожение оБьекта Ввода-Вывода;
    free(pdp->device_io); // Высвобождение памяти;
    reg_destroy((struct pdp_11_t *) pdp); // Уничтожение оБьекта Регистры;
    free(pdp->regist);
    command_destroy((struct command_t **)
                        pdp->command); // Уничтожение оБьекта машинные команды;
    free(pdp->command);
}
//////////////////////////////////////////////////////
