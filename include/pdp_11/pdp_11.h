...
byte_t b_read(struct pdp_11_t* , address_byte_t);// читаем байт по adr и возвращаем его;

void w_write(struct pdp_11_t* , address_word_t, word_t);// пишем значение (слово) val по адрессу adr;
word_t w_read(struct pdp_11_t* , address_word_t);// читаем слово по адрессу adr и возвращаем его;
