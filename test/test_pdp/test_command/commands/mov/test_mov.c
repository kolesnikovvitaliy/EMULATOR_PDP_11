...
test_mov(struct pdp_11_t *pdp, word_t *ptr_pc)
{
    ...
    run_command = __ptr_command(
        (pdp_11_t *) pdp, (command_t **) ptr_pdp->command, *ptr_pc);
...
//  * момент жестко задано по адресу 01000) и запускает их выполнение для
проверки
...
//  * @todo Раскомментировать и интегрировать основной цикл выполнения
do_command.
//  */
//
// command_t *
// __ptr_command(pdp_11_t *pdp, command_t **commands, const address_word_t
// addr)
// {
...
//     /*PRINT_RESULT(
//         "\n%s\n",
//         __ptr_command((pdp_11_t *) pdp, (command_t **) ptr_pdp->command,
//         addr)
//             ->name);
//     */
//     PRINT_RESULT("\nTEST_MOV_FIRST\n", "");
...
//     assert(strcmp((char *) run_command->name, "mov") == 0);
//
//     run_command->do_commands_command(pdp, addr, w_read(pdp, addr), (byte_t)
//     1);
//     // test_mov(pdp, ptr_pc);
//
//     WARNING("TEST HALT\n", "");
