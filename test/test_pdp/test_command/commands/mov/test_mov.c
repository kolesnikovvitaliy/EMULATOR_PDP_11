...
// {
//     //! < Сбрасываем процессор в исходное состояние: PC = 01000, регистры
//     R0-R6
//     //! очищены
//     pdp_reg_set_var(pdp, 7, 01000);
//     for (int i = 0; i <= 6; i++) {
...
//     pdp_reg_set_var(pdp, 05, 0200); ///< В R5 помещаем адрес источника
//     (0200)
//
...
//     op_code_t opcode = { { 0, 0 }, { 0, 0 } };
//     if (pdp) {
//         //! < Читаем инструкцию из памяти по заданному адресу и парсим
//         операнды opcode = __get_mr(pdp, (word_t) w_read(pdp, addr));
//     }
//
//     PRINT_RESULT("\r          ", "");
