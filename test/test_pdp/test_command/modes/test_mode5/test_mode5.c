...
#include <string.h>
/**
 * @brief Тестирование режима адресации 5 (Автодекрементный режим).
...
void
test_mode5(struct pdp_11_t *pdp)
{
    ...
    w_write(pdp, addr, (word_t) 0015301);

    pdp_reg_set_var(pdp, 3, 01004);
    w_write(pdp, 01002, (word_t) 0206);
    w_write(pdp, 0206, (word_t) 017);
