...
void
test_pc_reg_1(struct pdp_11_t *pdp)
{
...
void
test_valid_reg_1(struct pdp_11_t *pdp)
{
    //! < Очистка регистрового файла
    address_word_t addr = 01006;
    ...
    PRINT_RESULT("\r                            ", "");
    PRINT_RESULT("\x1b[F", "");
    TRACE_LOG("%s", "test_mode7_valid_reg ");
...
void
test_mode7(struct pdp_11_t *pdp)
{
    test_valid_reg_1(pdp);
    test_pc_reg_1(pdp);
    pdp_reg_clear(pdp);
    return;
}
