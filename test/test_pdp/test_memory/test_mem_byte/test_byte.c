...
void
test_rw_byte_b(struct pdp_11_t *pdp)
{
...
void
test_rw_word_b(struct pdp_11_t *pdp)
{
...
void
test_w2b_rword_b(struct pdp_11_t *pdp)
{
...
//--------------------------------------------------------------------
void
test_wword_r2b_b(struct pdp_11_t *pdp)
{

    address_word_t a;
    byte_t         b0, b1, res_b0, res_b1;
    word_t         w;
    TRACE_LOG("Пишем слово читаем 2 байта", "");
    a = 4; // другой адрес
    w = 0xa1b2;
    // little-endian, младшие разряды по меньшему адресу
    res_b0 = (byte_t) 0xb2;
...
{
    INFO_LOG("TEST MEMORY TYPE ==BYTE== START", "");

    test_rw_byte_b(pdp);
    test_rw_word_b(pdp);
    test_w2b_rword_b(pdp);
    test_wword_r2b_b(pdp);
    INFO_LOG("TEST MEMORY TYPE ==BYTE== WAS PASSED SUCCESSFULLY\n", "");
}
//////////////////////////////////////////////////////////////////////
